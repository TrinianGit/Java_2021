...
class BusLine implements BusLineInterface{
	...
	public Map<String, List<Position>> getLines() {
		Map <String, List<String>> Check = getIntersectionsWithLines();
		Map <String, List<Position>> BusLineRouteCopy = new HashMap<>(BusLineRoute);
		for (int i = 0; i < BusLines.size(); i++) {
			if (Check.get(BusLines.get(i)) == null) {
				BusLineRouteCopy.remove(BusLines.get(i));
			}
		}
		return BusLineRouteCopy;
	}
	...
	public Map<String, List<Position>> getIntersectionPositions() {
		...
		for (int i = 0; i < BusLines.size(); i++) {
			...
			if (ToMap.size() != 0) IntersectionPositions.put(actualBusLine, ToMap);
		}
	...
	public Map<String, List<String>> getIntersectionsWithLines() {
		...
		for (int i = 0; i < BusLines.size(); i++) {
			...
			if (ToMap.size() != 0) IntersectionWithLines.put(actualBusLine, ToMap);
		}
